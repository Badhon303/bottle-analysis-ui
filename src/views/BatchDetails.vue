<template>
  <div v-if="batch">
    ...
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="text-gray-500">
          {{ batch.supplier }} -
          <span class="uppercase">{{ batch.status }}</span>
        </p>
      </div>
      <div class="flex gap-2">
        ...
        <router-link
          to="/batches"
          class="px-4 py-2 bg-blue-700 hover:bg-blue-800 rounded-lg text-white text-sm font-medium"
        ...
        <p
          ...
          :class="{
            'text-green-500':
              ...
              summary?.total_bottles === batch.labeled_bottles,
          }"
        >
          {{ batch.labeled_bottles || 0 }}
        </p>
      </div>
      <div class="p-4 bg-white rounded-lg shadow dark:bg-gray-800">
        <h3 class="text-sm text-gray-500">Pending Review</h3>
        <p class="text-2xl font-bold text-yellow-500">
          {{ batch.pending_bottles || 0 }}
        </p>
      </div>
      <div class="p-4 bg-white rounded-lg shadow dark:bg-gray-800">
        <h3 class="text-sm text-gray-500">Impurity</h3>
        <p class="text-2xl font-bold text-red-500">
          {{ batch.impurity_percentage?.toFixed(2) || 0 }}%
        </p>
      </div>
    </div>
      ...
      <ul class="flex flex-wrap -mb-px text-sm font-medium text-center">
        <li class="me-2">
          <button
            ...
            :class="{
              'border-blue-600 text-blue-600': activeTab === 'bottles',
            }"
        ...
        <li class="me-2">
          <button
            ...
            :class="{
              'border-blue-600 text-blue-600': activeTab === 'clusters',
            }"
    ...
    <div v-if="activeTab === 'bottles'">
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        >
          ...
          <div class="p-2">
            <h5
              class="mb-1 text-sm font-bold tracking-tight text-gray-900 dark:text-white truncate"
            >
              {{ bottle.label_name || "Unknown" }}
            </h5>
            <p class="mb-1 text-xs text-gray-500">
              {{ (bottle.confidence * 100).toFixed(0) }}% Conf
            </p>
            <span
              :class="{
          ...
          >
            <button
              @click="openReassignModal(bottle)"
              class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-xs px-3 py-2"
            >
              Edit
            </button>
          </div>
    ...
    <div v-if="activeTab === 'clusters'">
      <div v-if="clusters.length === 0" class="text-center py-10 text-gray-500">
        No clusters found.
      </div>
      <div
      ...
      >
        <h3 class="text-lg font-bold mb-2">
          Cluster #{{ cluster.cluster_id }} ({{ cluster.count }} bottles)
        </h3>
        <div class="flex gap-2 overflow-x-auto mb-4 pb-2">
          <div
            v-for="(img, idx) in cluster.sample_images"
            :key="idx"
        ...
        <button
          @click="assignClusterLabel(cluster.cluster_id)"
