        ...
        >
          ...
          <ul class="space-y-2 text-sm text-gray-700 dark:text-gray-400">
            <li class="flex justify-between">
              ...
              <span class="font-bold">{{ results.total_bottles }}</span>
            </li>
            <li class="flex justify-between">
              <span>Matched:</span>
              <span class="text-green-600 font-bold">{{
                results.matched_bottles
              }}</span>
            </li>
            <li class="flex justify-between">
              <span>Unmatched:</span>
              <span class="text-red-600 font-bold">{{
                results.unmatched_bottles
              }}</span>
            </li>
          </ul>
        </div>
      ...
      <div class="lg:col-span-2">
        ...
        >
          <div v-if="results && results.visualization_path" class="w-full">
            <h3 class="text-lg font-bold mb-4 dark:text-white text-center">
              Visualization
            </h3>
            <img
              :src="getAssetUrl(results.visualization_path)"
              alt="Visualization"
          ...
          >
            ...
            <img
              :src="imagePreview"
              class="max-w-full h-auto rounded-lg opacity-50"
            />
          </div>
        ...
        <!-- Selection Controls -->
        <div
          v-if="results && results.clusters && results.clusters.length"
        ...
        <div
          v-if="
            results &&
            results.bottles &&
            results.bottles.filter((b) => b.status !== 'no_match').length > 0
          "
          class="mt-6"
        >
          <h3 class="text-xl font-bold mb-4 dark:text-white">
          ...
          >
            <div
              v-for="bottle in results.bottles.filter(
                (b) => b.status !== 'no_match',
              )"
              :key="bottle.id"
            ...
            >
              ...
              <p class="text-xs text-gray-500">Index: {{ bottle.id }}</p>
        ...
        >
          ...
          >
            ...
            <div
              class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border-l-4"
              :class="getClusterBorderClass(cluster.cluster_id)"
            >
              <div
                v-for="(bottleId, index) in cluster.bottle_ids"
                :key="bottleId"
                class="relative p-3 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 cursor-pointer transition-all"
                :class="{
                  'ring-4 ring-blue-500 dark:ring-blue-400': isBottleSelected(
                    getBottleIndex(bottleId),
                  ),
                }"
              ...
              >
                ...
                <input
                  type="checkbox"
                  :checked="isBottleSelected(getBottleIndex(bottleId))"
                ...
                <p class="text-xs text-gray-500">
                  Conf:
                  {{ (getBottleById(bottleId).confidence * 100).toFixed(0) }}%
                </p>
                <p class="text-xs text-gray-500">
                  Match:
                  {{
                    (getBottleById(bottleId).match_confidence * 100).toFixed(0)
                  }}%
                </p>
              </div>
            </div>
