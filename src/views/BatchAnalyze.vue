  ...
  <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1">
        ...
        >
          <h2
            class="text-xl font-bold mb-4 text-gray-900 dark:text-white flex items-center gap-2"
          >
            <svg
        ...
        >
          <h3
            class="text-lg font-bold mb-4 dark:text-white flex items-center gap-2"
          >
            <svg
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <span
                class="font-mono text-sm text-gray-900 dark:text-white truncate ml-2 max-w-[150px]"
            ...
            <div
              v-if="results.clusters && results.clusters.length"
            ...
            >
              ...
              >
              <span
                class="font-bold text-purple-600 dark:text-purple-400 text-xl"
                >{{ results.clusters.length }}</span
              >
            </div>
            <!-- Weight Summary (calculated from bottle weights) -->
            <div
              v-if="totalEstimatedWeight.grams > 0"
            ...
            >
              ...
              <div class="text-right">
                <span
                  class="font-bold text-cyan-600 dark:text-cyan-400 text-xl"
            ...
            >
              <h4
                class="text-sm font-bold mb-3 text-gray-700 dark:text-gray-300"
              >
                Label Summary
              </h4>
              <div class="space-y-2">
                <div
                  v-for="summary in labelSummary"
                  :key="summary.name"
                ...
                >
                  ...
                  <div class="flex gap-4 text-right">
                    ...
                    >
                    <span
                      class="text-cyan-600 dark:text-cyan-400 min-w-[60px]"
      ...
      <div class="lg:col-span-2">
        ...
        <div v-if="results">
          ...
          >
            <div class="flex items-center justify-between flex-wrap gap-4">
              ...
              <div class="flex gap-2">
                ...
                <button
                  @click="showCreateLabelForm = true"
          ...
          <div v-if="matchedBottles.length > 0" class="mb-6">
            <h3
              class="text-xl font-bold mb-4 dark:text-white flex items-center gap-2"
            >
              <svg
                class="w-5 h-5 text-green-500"
                fill="none"
            ...
            >
              <div
                ...
                :class="{
                  'ring-4 ring-blue-500 dark:ring-blue-400': isBottleSelected(
                    bottle.id,
                  ),
                }"
              ...
              >
                ...
                <input
                  type="checkbox"
                  :checked="isBottleSelected(bottle.id)"
                ...
                <div class="relative group mt-4">
                  <img
                    :src="getAssetUrl(bottle.image_path)"
                ...
                <!-- Bottle weight -->
                <p
                  v-if="bottle.label_weight_grams"
          ...
          <div v-if="results.clusters && results.clusters.length">
            ...
            >
              ...
              <div class="flex items-center mb-3">
                <div class="flex-grow">
                  <h4
                    class="text-lg font-semibold dark:text-white flex items-center gap-2"
                  >
                    <span
                      class="w-3 h-3 rounded-full"
                      :class="getClusterDotClass(cluster.cluster_id)"
                    ></span>
                    <span v-if="cluster.cluster_id === -1"
                      >Outliers (No Cluster)</span
                    >
                    <span v-else>Cluster {{ cluster.cluster_id }}</span>
                    <span
                      class="ml-2 text-sm font-normal text-gray-500 dark:text-gray-400"
                    >
                      ({{ cluster.bottle_count }}
                      {{ cluster.bottle_count === 1 ? "bottle" : "bottles" }})
                    </span>
                  </h4>
                </div>
                <div class="flex gap-2">
                  <button
                    @click="selectCluster(cluster.cluster_id, true)"
                    class="text-sm px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded"
                  >
                    Select All
                  </button>
                  <button
                    @click="selectCluster(cluster.cluster_id, false)"
              ...
              <div
                class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border-l-4"
                ...
                <div
                  ...
                  :class="{
                    'ring-4 ring-blue-500 dark:ring-blue-400':
                      isBottleSelected(bottleId),
                  }"
                  @click="toggleBottleSelection(bottleId)"
                >
                  <!-- Source image badge -->
                  <span
                    v-if="getBottleById(bottleId)"
                    class="absolute top-1 left-1 px-1.5 py-0.5 text-xs bg-gray-800/70 text-white rounded z-10"
                  >
                  ...
                  <input
                    type="checkbox"
                    :checked="isBottleSelected(bottleId)"
                    @click.stop="toggleBottleSelection(bottleId)"
                    class="absolute top-1 right-1 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 z-10"
                  />
                  <div class="relative group mt-4">
                    <img
                      :src="getAssetUrl(getBottleById(bottleId)?.image_path)"
                      class="w-full h-32 object-cover rounded mb-2"
                      :alt="'Bottle ' + bottleId"
                    />
                    <button
                      @click.stop="
                        openPreview(
                          getAssetUrl(getBottleById(bottleId)?.image_path),
                  ...
                  <p
                    v-if="getBottleById(bottleId)?.label_weight_grams"
                    class="text-xs text-cyan-600 dark:text-cyan-400 font-medium"
                  >
                  ...
                  <!-- Top similarity hint -->
                  <p
                    v-if="getBottleById(bottleId)?.similarities?.length"
                  ...
                  >
                    ...
                    {{ getBottleById(bottleId).similarities[0].label_name }} ({{
                      (
                        getBottleById(bottleId).similarities[0].similarity * 100
                      ).toFixed(0)
                    }}%)
                  </p>
                </div>
        ...
        >
          ...
          <p class="text-gray-500 dark:text-gray-400 text-center max-w-md">
            Select up to 50 images to analyze together. Bottles will be
            detected, matched against known labels, and similar unmatched
            bottles will be clustered across all images.
          </p>
        </div>
      </div>
    </div>
    ...
    <!-- Create Label Modal -->
    <div
      v-if="showCreateLabelForm"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
    >
      <div
        class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl max-w-md w-full mx-4"
      >
        <h3 class="text-xl font-bold mb-4 dark:text-white">
          Create Label from {{ selectedBottles.length }} Bottle(s)
        </h3>
        <form @submit.prevent="createLabelFromSelection" class="space-y-4">
          ...
          <div>
            ...
            <input
              type="number"
              v-model.number="newLabelForm.volume_ml"
              required
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
          </div>
          ...
          <div>
            <label
              class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
              >Empty Weight (grams)</label
            >
            <input
              type="number"
              step="0.1"
              v-model.number="newLabelForm.empty_weight_grams"
              required
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
          </div>
          <div class="flex gap-2 justify-end">
            <button
              type="button"
              @click="showCreateLabelForm = false"
    ...
    >
      ...
      >
        <h3 class="text-xl font-bold mb-4 dark:text-white">
          Add {{ selectedBottles.length }} Bottle(s) to Label
        </h3>
        <form @submit.prevent="addBottlesToExistingLabel" class="space-y-4">
          ...
          <div class="flex gap-2 justify-end">
            <button
              type="button"
              @click="showAddToLabelForm = false"
              class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg"
            >
              Cancel
            </button>
            <button
              type="submit"
              :disabled="addingToLabel || !selectedLabelId"
    ...
    >
      <div class="relative max-w-7xl max-h-screen p-4" @click.stop>
        ...
        <div class="mt-4 text-center text-white">
          ...
          <p
            v-if="previewImage.bottleId !== null"
            class="text-sm text-gray-300"
          >
            Bottle ID: {{ previewImage.bottleId }}
          </p>
        </div>
      </div>
    </div>
...
const analyzeImages = async () => {
  ...
  try {
    const response = await imagesApi.analyzeBatchImages(
      formData,
      (progressEvent) => {
...
const createLabelFromSelection = async () => {
  ...
  try {
    ...
    alert(
      `✓ Created label '${response.data.label_name}' from ${response.data.sample_count} bottles`,
    )
...
const addBottlesToExistingLabel = async () => {
  ...
  try {
    ...
    alert(
      "Failed to add bottles to label: " +
        (error.response?.data?.detail || error.message),
    )
  } finally {
